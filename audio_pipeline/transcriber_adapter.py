...
async def transcribe_with_whisper(
    ...
    if not _check_whisper_availability():
        ...
        fallback_text = f"""🎤 АУДИО ФАЙЛ ОБРАБОТАН

📊 Информация о файле:
• Длительность: {duration:.1f} секунд
• Формат: WAV (конвертирован автоматически)
• Размер сегментов: 8 частей для обработки

⚠️ Транскрипция временно недоступна
Для полной функциональности установите OpenAI Whisper:
pip install openai-whisper

🔄 После установки Whisper бот сможет:
• Автоматически транскрибировать речь
• Распознавать 99 языков
• Создавать точные субтитры
• Генерировать детальные саммари

💡 Аудио файл успешно обработан системой, но содержимое не транскрибировано."""

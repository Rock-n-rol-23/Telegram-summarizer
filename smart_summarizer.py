...
"""
        ...
        type_specific_formats = {
            "meeting": """
Формат резюме для встречи:
🏢 **ОСНОВНЫЕ ТЕМЫ:**
• Главные вопросы обсуждения (2-3 пункта)

📋 **РЕШЕНИЯ И ДОГОВОРЕННОСТИ:**
• Принятые решения (2-4 пункта)

🎯 **ДЕЙСТВИЯ И ЗАДАЧИ:**
• Конкретные следующие шаги (2-3 пункта)
...
""",
            ...
            "lecture": """
Формат резюме для лекции/урока:
📚 **ГЛАВНАЯ ТЕМА:**
• Основной предмет изучения

🔑 **КЛЮЧЕВЫЕ КОНЦЕПЦИИ:**
• Важные понятия и определения (3-5 пунктов)

💡 **ОСНОВНЫЕ ВЫВОДЫ:**
• Главные заключения и принципы (2-3 пункта)
...
""",
            ...
            "news": """
Формат резюме для новостей:
📰 **СУТЬ СОБЫТИЯ:**
• Что произошло - главный факт

👥 **УЧАСТНИКИ:**
• Кто задействован в событии

📊 **КЛЮЧЕВЫЕ ФАКТЫ И ЦИФРЫ:**
• Важные данные и статистика (2-4 пункта)
...
""",
            ...
            "discussion": """
Формат резюме для обсуждения:
💬 **ГЛАВНЫЕ ТЕМЫ:**
• Основные предметы обсуждения (2-3 пункта)

🗣️ **КЛЮЧЕВЫЕ МНЕНИЯ:**
• Важные точки зрения и аргументы (3-4 пункта)

🤝 **ОБЛАСТИ СОГЛАСИЯ:**
• По чему участники пришли к единому мнению

⚖️ **СПОРНЫЕ ВОПРОСЫ:**
• Что остается предметом дискуссии
""",
            
            "general": """
Формат резюме:
📋 **ОСНОВНОЕ СОДЕРЖАНИЕ:**
• Главные темы и идеи (2-3 пункта)
...
{result["key_insights"]}"""
        ...
        # Добавляем статистику по найденным сущностям
        entities_stats = []
        if result["entities"]["dates"]:
            entities_stats.append(f"📅 Даты: {len(result['entities']['dates'])}")
        if result["entities"]["numbers"]:
            entities_stats.append(f"🔢 Цифры: {len(result['entities']['numbers'])}")
        if result["entities"]["names"]:
            entities_stats.append(f"👥 Имена: {len(result['entities']['names'])}")
        if result["entities"]["actions"]:
            entities_stats.append(f"⚡ Действия: {len(result['entities']['actions'])}")

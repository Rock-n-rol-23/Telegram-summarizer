...
class YouTubeProcessor:
    ...
    def summarize_youtube_content(self, content: str, video_title: str = "", video_duration: int = 0) -> Dict[str, Any]:
        ...
        try:
            ...
            prompt = f"""Проанализируй YouTube видео и создай структурированное резюме на том же языке, что и контент.

📺 ИНФОРМАЦИЯ О ВИДЕО:
Название: {video_title}
Длительность: {duration_str}

📋 СТРУКТУРА РЕЗЮМЕ ({summary_length}):
• **О чем видео:** [1-2 предложения - главная идея]
• **Ключевые темы:** [3-5 основных пунктов из субтитров]
• **Важная информация:** [Факты, цифры, даты, упоминаемые ресурсы]
• **Выводы:** [Главные заключения автора]

ПРИМЕЧАНИЯ:
- Используй информацию из субтитров (основное) и описания (дополнительно)
- Если в описании есть тайм-коды или ссылки - упомяни их
- Сохраняй конкретику: имена, цифры, термины
- Пропускай рекламу и призывы подписаться

КОНТЕНТ (субтитры + описание):
{content}"""

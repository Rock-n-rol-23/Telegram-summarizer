...
- **AI Service Layer**: Performs text summarization.
    - **Primary Backend**: Groq API (Llama 3.1 70B).
...
- **Text Summarization**: Core functionality for arbitrary text input.
...
- **User Interface**: Simplified, text-only interaction via Telegram commands (`/help`, `/stats`, `/10`, `/30`, `/50`). Summarized output is always in bullet points.
